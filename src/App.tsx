...
<head>
    ...
    <title>Ratiba ya Afya-Lishe</title>
    <style>
        body { font-family: -apple-system, blinkmacsystemfont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; line-height: 1.6; color: #333; max-width: 800px; margin: 20px auto; padding: 20px; background: #f0fdf4; }
        .header { text-align: center; margin-bottom: 40px; }
        h1 { color: #064e3b; margin-bottom: 5px; font-weight: 900; letter-spacing: -1px; }
        .sub { color: #059669; font-weight: bold; font-size: 14px; }
        .program-card { background: #065f46; color: white; padding: 30px; border-radius: 24px; margin-bottom: 30px; box-shadow: 0 10px 25px -5px rgba(6, 95, 70, 0.3); }
        .program-card h2 { margin: 0; font-size: 24px; }
        .program-card p { opacity: 0.8; margin: 10px 0 0; font-size: 14px; }
        .day-box { background: white; border: 1px solid #e5e7eb; border-radius: 20px; padding: 25px; margin-bottom: 25px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); }
        .day-header { font-size: 18px; font-weight: 900; color: #065f46; margin-bottom: 20px; text-transform: uppercase; border-left: 5px solid #10b981; padding-left: 15px; letter-spacing: 1px; }
        .session { border-bottom: 1px solid #f3f4f6; padding: 15px 0; display: flex; gap: 20px; }
        .session:last-child { border-bottom: none; }
        .time-col { width: 100px; flex-shrink: 0; }
        .t-lbl { font-size: 10px; font-weight: 800; color: #6b7280; text-transform: uppercase; }
        .t-val { font-size: 14px; font-weight: 800; color: #065f46; }
        .content-col { flex: 1; }
        .title { font-weight: 800; color: #111827; font-size: 15px; margin-bottom: 4px; }
        .desc { font-size: 13px; color: #4b5563; margin-bottom: 8px; }
        .pill { background: #d1fae5; color: #065f46; padding: 4px 12px; border-radius: 100px; font-size: 11px; font-weight: 800; display: inline-block; border: 1px solid #a7f3d0; }
        .footer { text-align: center; margin-top: 50px; font-size: 12px; color: #9ca3af; padding: 20px; border-top: 1px dashed #d1d5db; }
        @media print { body { background: white; } .program-card { background: #f0fdf4 !important; color: #065f46 !important; border: 2px solid #065f46; box-shadow: none; } }
    </style>
</head>
<body>
    <div class="header">
        <h1>AFYA-LISHE</h1>
        <div class="sub">JOS NATURAL HERBS - TIBA ASILI YA UHAKIKA</div>
    </div>

    <div class="program-card">
        <h2>${ratiba.title}</h2>
        <p>Programu maalum ya mwezi mmoja (Mzunguko wa siku 7 kwa wiki 4).</p>
    </div>
    
    ${ratiba.days.map(d => `
        <div class="day-box">
            <div class="day-header">SIKU YA ${d.day}</div>
            ${d.sessions.map(s => `
                <div class="session">
                    <div class="time-col">
                        <div class="t-lbl">${s.lbl}</div>
                        <div class="t-val">${s.t}</div>
                    </div>
    ...
    <div class="footer">
        Imetengenezwa na Afya-Lishe App &copy; 2026. <br>
        Kwa ushauri zaidi wasiliana na wataalamu wetu.
    </div>
</body>
</html>`;
    ...
    a.href = url;
    a.download = `Ratiba_Afya_Lishe_${user.lastDiagCondition}.html`;
          ...
          <motion.div key="home" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="sc on" id="s-home">
            <nav className="topbar">
              ...
              <div className="flex items-center gap-2">
                <button 
                  onClick={() => {
                    navigator.clipboard.writeText('https://ais-pre-w6tdeh2cwkanworwne5mvs-443460401903.europe-west2.run.app');
                    alert('Link ya App imenakiliwa! Unaweza kuipaste kwenye Browser au WhatsApp ya simu yako.');
                  }}
                  className="bg-emerald-100 text-emerald-700 px-3 py-1.5 rounded-full active:scale-95 transition-all flex items-center gap-1 border border-emerald-200"
                >
